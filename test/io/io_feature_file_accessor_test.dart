  ...
  group('Matcher', () {
    ...
    group('with RegExp', () {
      test('does not list directories', () async {
        expect(await indexer.listFiles(RegExp('test/test_resources/')), []);
      });

      test('does not throw error for weird paths', () async {
        expect(await indexer.listFiles(RegExp('nonexistentpath')), []);
      });
    });

    group('Glob', () {
      test('does not return directories', () async {
        expect(await indexer.listFiles(Glob('test/test_resources/')), []);
      });

      test('does not throw error for weird paths', () async {
        expect(await indexer.listFiles(Glob('nonexistentpath')), []);
      });
    });

    group('String', () {
      test('does not return directories', () async {
        expect(await indexer.listFiles('test/test_resources/'), []);
      });

      test('does not throw error for weird paths', () async {
        expect(await indexer.listFiles('nonexistentpath'), []);
      });
    });
  });
