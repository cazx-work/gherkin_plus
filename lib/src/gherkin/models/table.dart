...
class GherkinTable {
      ...
      ...rows.map((row) {
        ...
        if (header != null) {
          for (var i = 0; i < header!.columns.length; i += 1) {
            map[header!.columns.toList().elementAt(
              i,
          ...
          for (var i = 0; i < row.columns.length; i += 1) {
            map[i.toString()] = row.columns.length > i
                ? row.columns.elementAt(i)
                : null;
          }
        }
  ...
  factory GherkinTable.fromJson(String json) {
    final data = (jsonDecode(json) as List<dynamic>).map(
    ...
    final headerRow = data.isNotEmpty
        ...
        : null;
    final rows = data.map(
      (x) => TableRow(x.values.cast<String?>(), 1, isHeaderRow: false),
    );
