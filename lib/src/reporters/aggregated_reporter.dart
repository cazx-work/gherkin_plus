...
class AggregatedReporter extends FullReporter
  ...
  ReportActionHandler<TestMessage> get test => ReportActionHandler(
    onStarted: ([_]) =>
        invokeReporters<TestReporter>((r) => r.test.onStarted.invoke()),
    onFinished: ([_]) =>
        invokeReporters<TestReporter>((r) => r.test.onFinished.invoke()),
  );
  ...
  ReportActionHandler<StepMessage> get step => ReportActionHandler(
    onStarted: ([message]) =>
        invokeReporters<StepReporter>((r) => r.step.onStarted.invoke(message)),
    onFinished: ([message]) =>
        invokeReporters<StepReporter>((r) => r.step.onFinished.invoke(message)),
  );
